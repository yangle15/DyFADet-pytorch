...
class HacsDataset(Dataset):
    def __init__(
        ...
        # file path
        feat_folder = copy.deepcopy(feat_folder)
        if backbone_type != 'tsp':
            if isinstance(feat_folder, (list, tuple)):
                feat_folder[0] = os.path.join(feat_folder[0], split[0])
            else:
        ...
        if self.backbone_type == 'i3d':
            ...
            if self.backbone_type == 'slowfast':
                if isinstance(self.feat_folder, (list, tuple)):
                    ...
                    # self.feat_folder[0] = os.path.join(self.feat_folder[0], self.split[0])
                    filename = os.path.join(self.feat_folder[0], video_item['id'] + self.file_ext[0])
                    # print(filename)
                    ...
                    filename = os.path.join(self.feat_folder[1],
                                'v_' + video_item['id'] + self.file_ext[1])
                    ...
                    feats = np.concatenate([feats, feats_mae],  axis=1)        
