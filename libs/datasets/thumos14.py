...
class THUMOS14Dataset(Dataset):
    ...
    def _load_json_db(self, json_file):
        ...
        for key, value in json_db.items():
            ...
            if isinstance(self.feat_folder, (list, tuple)):
                feat_file = os.path.join(self.feat_folder[0],
    ...
    def __getitem__(self, idx):
        ...
        # load features
        if isinstance(self.feat_folder, (list, tuple)): # if with multiple kinds of features: the first is i3d and the second is videomaev2
            assert len(self.feat_folder) < 3

            filename = os.path.join(self.feat_folder[0],
                                self.file_prefix + video_item['id'] + self.file_ext) 
            feats_i3d = np.load(filename).astype(np.float32)
            feats_rgb = feats_i3d[:,:1024]
            feats_flow = feats_i3d[:,1024:2048]

            filename = os.path.join(self.feat_folder[1],
                                self.file_prefix + video_item['id'] + self.file_ext) 
            feats_mae = np.load(filename).astype(np.float32)
            ...
            feats = np.concatenate([feats_flow, feats_mae],  axis=1)        
